...
2) Storage hooks (RAM example, tests only):
    ...
    static int ram_write(const void *buf, size_t len, void *user) { (void)user; if (len > sizeof(ram_buf)) return -1; memcpy(ram_buf, buf, len); ram_len = len; return 0; }
    static int ram_read(void *buf, size_t len, void *user)  { (void)user; if (ram_len == 0) return -1; if (len != ram_len) return -1; memcpy(buf, ram_buf, len); return 0; }
    static const loxseq_storage_t storage = { .write_checkpoint = ram_write, .read_checkpoint = ram_read, .erase_checkpoint = 0, .user = 0 };
...
4) Main loop:
    ...
    for (;;) { (void)loxseq_tick(&seq, platform_now_ms()); }
